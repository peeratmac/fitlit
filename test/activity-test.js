...
describe('ACTIVITY', () => {
  it('should calculate the miles a user has walked on a given day', () => {
    expect(activity.calculateMilesWalked('2019/06/20', user1ID)).to.equal(11.8);
  });
  ...
  it('should calculate how many minutes active did they average for a given week', () => {
    expect(activity.getWeekAverageActivity('2019/06/21', user1ID)).to.equal(12);
  });

  it('should check if a user reached their step goal on a given day', () => {
    expect(activity.checkStepGoal('2019/06/20', user1ID)).to.equal(true);
  });

  it('should find all the days a user exceeded their step goal', () => {
    let days = [];
    expect(activity.findExceedStepGoal(user1ID)).to.eql(days);
  });

  it('should find a user\'s all-time stair climbing record', () => {
    expect(activity.getMostStairsClimbed(user1ID)).to.equal(36);
  });

  it.only('should calculate the all user average steps, active minutes and flights of stairs climbed for a given day', () => {
    expect(activity.getAllUserAverage('2019/06/20', 'numSteps')).to.equal(8711.6);
    expect(activity.getAllUserAverage('2019/06/20', 'minutesActive')).to.equal(180.6);
    expect(activity.getAllUserAverage('2019/06/20', 'flightsOfStairs')).to.equal(25.5);
  });
